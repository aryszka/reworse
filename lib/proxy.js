(function () {
    ...
    var mapRequest = function (proxy, req) {
        ...
        if (req.headers) {
            rawHeaders = Headers.merge(
                Headers.canonical(Headers.toRaw(req.headers)),
                rawHeaders
            );
        }

        var preq = proxyRequest(proxy, req.method, req.url, rawHeaders);
    ...
    var mapResponse = function (pres, res) {
        ...
        var head = {
            ...
            headers:    Headers.mapRaw(pres.rawHeaders)
        };
        ...
        if (noData(pres)) {
            ...
            pres.on("data", function (data) {
                ...
                res.emit("data", dataContext);
                res.write(dataContext.data);
            });

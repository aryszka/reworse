(function () {
    ...
    var defaultHandler = function (options, message) {
        ...
        options.out(message.message || message);
    };
    ...
    // creates a notification handler, that:
    // - ignores external econnreset from the clients in non-verbose mode
    // - ignores isp polling to hosts in non-verbose mode
    // - ignores connection failure to hosts in non-verbose mode
    // - prints user message for fake certificates
    // - prints user message for http fallback
    // - prints full messages in verbose mode
    // - prints simplified messages in non-verbose mode
    var create = function (options) {
        ...
        return function (message) {
            ...
            if (!handled) {
                defaultHandler(options, message);
            }
        };
    };

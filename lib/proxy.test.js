suite("proxy", function () {
    ...
    test("conditions request headers", function (done) {
        ...
        proxyRequests.on("request", function (preq) {
            var canonical = Headers.canonical(rawHeaders);
    ...
    test("maps request headers", function (done) {
        ...
        var rawHeaders = [
            "Some-Header-0",             "some value 0",
            "Some-Conflicting-Header-1", "some value 11"
        ];

        var parsedHeaders = {
            "Some-Conflicting-Header-1": "some value 12",
            "some-parsed-header-2":      "some value 2",
            "Some-Parsed-Header-2":      "some value 3"
        };
        ...
        var testHeaders = Headers.merge(
            Headers.canonical(Headers.toRaw(parsedHeaders)),
            rawHeaders
        );

        proxyRequests.on("request", function (preq) {
            for (var i = 0; i < testHeaders.length; i += 2) {
                assert(preq.options.headers[testHeaders[i]] === testHeaders[i + 1]);
            }
